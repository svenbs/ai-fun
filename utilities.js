...
/**
 * Scan Room for Sources, Creeps, etc.
 */
Room.prototype.scan = function() {
	...
	for (var i in room.sources) {
		...
		// Calculate number of worker modules needed to fully harvest sources
		var energyRate = source.energyCapacity / ENERGY_REGEN_TIME;
		sourceMemory.maxWorkParts = Math.ceil(energyRate / 2 * 1.2);
		...
		for (var t in terrain) {
			var tile = terrain[t];
			// If tile is source skip this iteration
			if (tile.x == source.pos.x && tile.y == source.pos.y) {
				continue;
			}

			if (tile.terrain == 'plain' || tile.terrain == 'swamp') {
				// If tile is blocking Structure skip this iteration
				// This is not working, because tile is only one object at this position
				// @todo: Try with isArray false
				/*
				if (tile.type == 'structure') {
					for (var blockingStructure in OBSTACLE_OBJECT_TYPES) {
						if (tile.structure.structureType == blockingStructure) {
							continue;
						}
					}
				}
				*/
				free++;
				// needed to decide a drop off spot for energy later
		...
		for (var t in harvesters) {
			...
			if (harvester.memory.fixedSource == source.id) {
				sourceMemory.harvesters.push(harvester.id);
				// @todo: getBodyParts function
		...
		while (sourceMemory.harvesters.length > 0 && (sourceMemory.harvesters.length > free || totalWork - utilities.getBodyParts(harvester).work >= sourceMemory.maxWorkParts)) {
			...
			if (harvester) {
				...
				delete harvester.memory.fiexedDropoffSpot;
				totalWork -= utilities.getBodyParts(harvester).work;
			}
			harvester = Game.getObjectById(sourceMemory.harvesters[sourceMemory.harvester.lenght -1]);
		}

		// Assign free harvesters
		for (var t in harvesters) {
			if (sourceMemory.harvesters.lenght >= free || totalWork >= sourceMemory.maxWorkParts) {
				break;
			}
			...
			if (!harvester.memory.fixedSource) {
				sourceMemory.harvesters.push(harvester.id);
				harvester.memory.fixedSource == source.id;
				delete harvester.memory.fixedTarget;
				delete harvester.memory.fiexedDropoffSpot;
			}
		}
		...
		if (!sourceMemory.dropoffSpot) {
			...
			for (var t in terrain) {
				...
				if (tile.terrain == 'plain' || tile.terrain == 'swamp') {
					...
					// @todo: Check for blocking structure at position.
					if (count > bestCount) {
						bestCount = count;
						best = tile;
					}
		...
		room.memory.sources[id] = sourceMemory;
	}
...
var utilities = {
	...
	// Get target ID of closest target.
	...
	/**
	 * Find Best Option
	 * options array consists of json structure =>
		priority: [number],
	 ...
	 * IF Priorities are equal higher weight wins
	 */
