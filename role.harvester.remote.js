...
Creep.prototype.performBuildRoad = function() {
	...
	if (creep.pos.roomName != targetPosition.roomName) {
		...
		var structure = creep.pos.lookFor(LOOK_STRUCTURES);
		...
		else if (constructionSites && constructionSites.length > 0) {
		...
		else if (!actionTaken && structure.structureType == STRUCTURE_ROAD && structure.hits < structure.hitsMax - workParts * 100) {
			creep.repair(structure);
			actionTaken = true;
			// If structure is especially damaged, stay here to repair
			if (structure.hist < structure.hitsMax - workParts * 2 * 100) {
				creep.repair(structure);
				return true;
			}
		}
...
Creep.prototype.performRemoteHarvesterDelivery = function() {
	...
	if (!target || _.sum(target.store) + creep.carry.energy >= target.storeCapacity) {
		...
		if (creep.drop(RESOURCE_ENERGY) == OK) {
			Memory.rooms[targetPosition.roomName].remoteHarvesting[creep.memory.source].revenue += creep.carry.energy;
			return true;
		}
	}
